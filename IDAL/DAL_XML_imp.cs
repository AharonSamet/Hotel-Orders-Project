      ...
      //};
        ...
        public DAL_XML_imp()
        {
            try
            {
                ...
                if (!File.Exists(XmlConfigurations.BankBranchesATMPath))
                {
                    XmlConfigurations.BankBranchesRoot = new XElement("BankBranches");
                    XmlConfigurations.BankBranchesRoot.Save(XmlConfigurations.BankBranchesATMPath);
                }
                XmlConfigurations.OrderRoot = XElement.Load(XmlConfigurations.OrdersPath);
                XmlConfigurations.BankBranchesRoot = XElement.Load(XmlConfigurations.BankBranchesATMPath);
        ...
        public void AccommodationStarsUpdate(int accommodationKey, string hostKey, int stars)
        {
            if (_hosts.All(x => x.Id != hostKey))
                throw new HostsException("Host doesn't exist");
        ...
        public void AccommodationSumOfUnitsUpdate(int accommodationKey, string hostKey, int sumOfUnits)
        {
            if (_hosts.All(x => x.Id != hostKey))
                throw new HostsException("Host doesn't exist");
            ...
            switch (detailsInAccommodationUpdate)
            {
                case "Stars":
                    foreach (var getAccommodation in from getHost in _hosts
                        ...
                        getAccommodation.Stars = (int)newValue.Clone();
                    break;
                case "SumOfUnits":
                    foreach (var getAccommodation in from getHost in _hosts
                        where getHost.Id == hostKey
                        from getAccommodation in getHost.HostAccommodationsList
                        where getAccommodation.AccommodationKey == accommodationKey
                        select getAccommodation) getAccommodation.SumOfUnits = (int)newValue.Clone();
                    break;
                default:
        ...
        public IEnumerable<BankBranch> GetAllBankBranches(Func<BankBranch, bool> predicate = null)
        {
            try
            {
                return (from bankBranches in XmlConfigurations.BankBranchesRoot.Elements()
                    ...
                    {
                        BankNumber = Convert.ToString(bankBranches.Element("קוד_בנק")?.Value.Trim()),
                        BankName = Convert.ToString(bankBranches.Element("שם_בנק")?.Value.Trim()),
                        BranchNumber = Convert.ToString(bankBranches.Element("קוד_סניף")?.Value.Trim()),
                        BranchAddress = new Address
                        {
                            Street = Convert.ToString(bankBranches.Element("כתובת_ה-ATM")?.Value.Trim()),//Element("Street")?.Value),
                            Building = "Empty for now",
                            City = Convert.ToString(bankBranches.Element("ישוב")?.Value),
                            ZipCode = Convert.ToString(bankBranches.Element("קואורדינטת_X")?.Value.Trim()),
                            Country = "Israel",
                        },
