...
namespace BL
{
    public class BlImp : IBL
    {
        ...
        {
            newGuestRequest.GuestRequestKey++;
         ...
         //   var sumOfDays = newGuestRequest.CheckOutDate.Subtract(newGuestRequest.CheckInDate);
           // if (sumOfDays.Days < 1)
            ...
            if (!ValidDate(newGuestRequest.CheckInDate, newGuestRequest.CheckOutDate))
                throw new Exception("date not correct"/*"You cannot place an order for more than eleven months ahead"*/);
        ...
        /// <param name="sorted"></param>
        /// <returns></returns>
        public IEnumerable<IGrouping<int, GuestRequest>> GetGuestRequestGroupBySumOfHosted(bool sorted = false)
        {
            return sorted
        ...
        private static bool ValidDate(DateTime first, DateTime end)
        {
